...
const { assertNoMutationIntent, assertReadOnlyContext, assertSuggestionValid } = require('../src/services/ai/automation/automationGuard');
const { validateAutomationSuggestion } = require('../src/services/ai/automation/automationContract');

describe('Automation Gate', () => {
  it('should reject mutation attempts in prompt', () => {
    expect(() => assertNoMutationIntent('Please delete invoice')).toThrow('Mutation intent detected');
  ...
  it('should only allow GET/read-only methods', () => {
    ...
    expect(() => assertReadOnlyContext({ method: 'POST' })).toThrow('Only GET/read-only methods are allowed');
    expect(() => assertReadOnlyContext({ method: 'DELETE' })).toThrow('Only GET/read-only methods are allowed');
  });
  ...
  it('should validate required suggestion fields', () => {
    ...
    expect(() => assertSuggestionValid(missingField)).toThrow('Missing required field: title');
