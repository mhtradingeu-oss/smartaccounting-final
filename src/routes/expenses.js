...
router.get('/', requireRole(['admin', 'accountant', 'auditor', 'viewer']), async (req, res, next) => {
  try {
    ...
    const { rows: expenses, count } = await expenseService.listExpenses(req.companyId, pagination);
    res.status(200).json({
      success: true,
      expenses,
...
router.get('/:expenseId', requireRole(['admin', 'accountant', 'auditor', 'viewer']), async (req, res, next) => {
  try {
...
router.patch('/:expenseId/status', requireRole(['admin', 'accountant']), async (req, res, next) => {
  try {
    ...
    if (!expense) {
      return res.status(404).json({ success: false, message: 'Expense not found or invalid status transition' });
    }
    res.status(200).json({ success: true, expense });
  } catch (error) {
    next(error);
