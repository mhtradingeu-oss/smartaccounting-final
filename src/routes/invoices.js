...
const invoiceCreateQueues = new Map();

const withInvoiceCreateQueue = async (key, fn) => {
  const previous = invoiceCreateQueues.get(key) || Promise.resolve();
  ...
  invoiceCreateQueues.set(key, queueTail);
  ...
  try {
    ...
    release();
    if (invoiceCreateQueues.get(key) === queueTail) {
      invoiceCreateQueues.delete(key);
    }
  }
};
...
router.post('/', requireRole(['admin', 'accountant']), async (req, res, next) => {
  const createInvoiceRequest = async () => {
    ...
    // Invoice numbers are accounting-critical and must be generated by invoiceService
    // when the client did not explicitly provide one.
    if (!req.body?.invoiceNumber) {
      delete normalizedData.invoiceNumber;
    }
  ...
  try {
    const lockKey = `invoice-create:${req.companyId}`;
    await withInvoiceCreateQueue(lockKey, createInvoiceRequest);
  } catch (error) {
    next(error);
  }
});
