...
async function aiReadGateway(input)
  ...
  // UI-9.9 SINGLE BRAIN DIRECT EXECUTION
  if (input?.type === 'CFO_INSIGHT') {
    return await cfoEngine.analyze(input.payload);
  }
 {

  // UI-8.6 ROUTING LAYER (CFO / AI)
  ...
  if (routed) return routed;
 {
  ...
  const { executeAIEvent } = require('./eventBus');
...
// LEGACY CFO REMOVED UI-9 FINAL

/**
 * UI-8.5 AUTO CFO HOOK
 * If CFO payload detected → route through AI decision engine
 */
