...
| --- | --- | --- | --- |
| Immutable audit trail | Audit logs (JSON + CSV + PDF) | `GET /api/exports/audit-logs` | Returns hashed entries filtered by `companyId` + optional `from`/`to`. JSON contains all fields; CSV mirrors the same columns; PDF is print-ready for audits. Sample JSON/CSV in `docs/operations/export-samples/audit-logs-example.*`. |
| Accounting journaling | Accounting records (JSON + CSV + PDF) | `GET /api/exports/accounting-records` | Returns invoices and expenses grouped by company and time range; CSV summary includes record type, amounts, and embedded payload metadata; PDF is print-ready. Sample JSON in `docs/operations/export-samples/accounting-records-example.json`. |
| VAT evidence | VAT summaries (JSON + CSV + PDF) | `GET /api/exports/vat-summaries` | Pulls `tax_reports` entries so filings can be verified against `UStVA` data; includes `generatedAt`, `submittedAt`, and the raw `data` payload; PDF is print-ready. Sample path `docs/operations/export-samples/vat-summaries-example.json`. |
