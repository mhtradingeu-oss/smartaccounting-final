...
- Use the root-level `VERSION` file as the canonical SemVer source of truth for the backend runtime and tie its contents to the release tag; keep `package.json` aligned but let the file drive runtime metadata.
...
- These values populate `src/config/buildMetadata.js`, which now sources `version` from `VERSION` and exposes `version`, `packageVersion` (alias), `commitHash`, `buildTimestamp`, `imageDigest`, `nodeVersion`, and `environment`.
...
- `/api/system/version` is a read-only endpoint that returns a curated metadata document (`version`, `gitSha`, `buildDate`, and `environment`) so monitors and auditors know exactly which release is running.
