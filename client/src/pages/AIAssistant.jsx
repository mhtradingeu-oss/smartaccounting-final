...
const AIAssistant = () => {
  ...
  const handleIntent = async (intentId, options = {}) => {
    ...
    try {
      ...
      setAskError(err.response?.data?.error || err.message || 'Unable to reach the assistant.');
    } finally {
      setIsAsking(false);
    }
  };
  ...
  if (!isAIAssistantEnabled()) {
    return (
      <EmptyState
        ...
        description="Enable the AI_ASSISTANT_ENABLED flag to expose the conversational advisor."
        action={
          <Button onClick={() => window.location.assign('/dashboard')} variant="primary">
            Return to dashboard
          </Button>
        }
      />
  ...
  if (contextError) {
    return (
      <EmptyState
        ...
        description={contextError?.response?.data?.error || contextError?.message || 'Please try again later.'}
        action={
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            A conversational, read-only advisor that highlights issues and refers to explainable insights.
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{context?.invoices?.length ?? 0} records</span>
          </div>
          {Object.keys(invoiceStatusBreakdown).length ? (
            ...
            <p className="text-xs text-gray-400">No invoices available yet.</p>
          )}
        </Card>
        ...
        <Card>
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{context?.expenses?.length ?? 0} records</span>
          </div>
        ...
        <Card className="lg:col-span-1">
          ...
          {context?.bankTransactions?.length ? (
            <p className="text-xs text-gray-500 mt-2">
              Latest: {context.bankTransactions[0].description} · {formatDate(context.bankTransactions[0].transactionDate)} · {formatCurrency(context.bankTransactions[0].amount, context.bankTransactions[0].currency)}
            </p>
          ) : (
            <p className="text-xs text-gray-400 mt-2">Upload bank data to surface reconciliation insights.</p>
          )}
        </Card>
          ...
          {context?.insights?.length ? (
            <div className="space-y-3">
              {context.insights.map((insight) => (
                <div key={insight.id} className="rounded-lg border border-gray-100 p-3 bg-gray-50">
                  <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-gray-400">No AI insights yet. The model will analyze data over time.</p>
          )}
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <Card className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                The assistant references invoices, expenses, bank statements, and AI insights. Every answer is grounded and audit logged.
              </p>
            </div>
            <span className="text-xs text-gray-400">Read-only advisor</span>
          </div>
          <div className="space-y-4">
            {messages.map((message) => (
              <div
                ...
                className={`rounded-lg border ${
                  message.speaker === 'assistant' ? 'border-gray-200 bg-white' : 'border-primary-400 bg-primary-50'
                } p-4`}
              >
        ...
        <Card className="space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Every session is anchored in an immutable audit log entry. Questions and answers are recorded with hashed prompts.
          </p>
          <p className="text-xs text-gray-500">Session tracked as {sessionId ? sessionId : 'pending...'}</p>
