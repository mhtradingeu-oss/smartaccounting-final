...
const OCRPreview = () => {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const message =
        ...
        'The preview could not be generated. Please try again later.';
  ...
  return (
    <div className="space-y-6">
      ...
      {!ocrPreviewEnabled ? (
        <section className="space-y-3 rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-8 text-center text-sm text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-900">
          <p className="text-lg font-semibold text-gray-900 dark:text-white">Preview disabled</p>
          <p>
            OCR preview is currently turned off. Enable `OCR_PREVIEW_ENABLED` to try the read-only
            experience.
          </p>
        </section>
      ) : (
